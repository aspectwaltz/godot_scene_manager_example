...
uniform vec2 amplitude = vec2(0.005, 0.005);
uniform vec2 frequency = vec2(0.1, 0.1);
uniform vec2 time_factor = vec2(10.0, 10.0);

uniform sampler2D uv_offset_texture : hint_black;
uniform vec2 uv_offset_scale = vec2(0.05, 0.05);
uniform float uv_offset_time_scale = 10.0;
uniform float uv_offset_amplitude = 0.5;
...
void fragment() {
	...
	EMISSION += textureLod(SCREEN_TEXTURE,ref_ofs,ROUGHNESS * 4.0).rgb * (1.0);// - ALPHA);
	ALBEDO *= ALPHA;
	ALPHA = 0.9;
}